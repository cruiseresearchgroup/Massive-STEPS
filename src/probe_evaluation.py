# Modified from: https://github.com/wesg52/world-models/blob/main/probes/evaluation.py
...
def haversine_distance(true, predicted):
    ...
    a = np.sin(dlat / 2) ** 2 + np.cos(predicted[:, 0]) * np.cos(true[:, 0]) * np.sin(dlon / 2) ** 2
...
def pairwise_haversine_distance(true, predicted):
    ...
    a = (
        torch.sin(dlat / 2) ** 2
        + torch.cos(predicted[None, :, 0]) * torch.cos(true[:, None, 0]) * torch.sin(dlon / 2) ** 2
    )
